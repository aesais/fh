...
export const LANUGAGES = ['en', 'lt', 'pl', 'pl_en'];
...
export class LanguageResiterer {
  ...
  public registerLanguags(instance) {
    LANUGAGES.forEach(async (lang) => {
      const fileLang = lang.split('_')[0];
      const file = `${instance.constructor.name}.${fileLang}`;
      try {
        if (instance.constructor.name) {
          const translation = await require(`../inputs/i18n/${file}.js`);
